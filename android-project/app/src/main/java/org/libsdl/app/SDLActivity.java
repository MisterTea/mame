...
*/
    ...
    protected void onDestroy() {
        ...
        if (SDLActivity.mSDLThread != null) {
            ...
            // Wait briefly for "SDLThread" to end. An unbounded join on the UI
            // thread ANRs when native code is blocked (e.g. Discord pump_until).
            try {
                SDLActivity.mSDLThread.join(1500);
                if (SDLActivity.mSDLThread.isAlive()) {
                    Log.w(TAG, "SDLThread still alive after join timeout; continuing onDestroy");
                }
            } catch(Exception e) {
...
*/
    ...
    public static boolean isTextInputEvent(KeyEvent event) {
        ...
        // MAME binds digits / space / modifiers as gameplay controls (Start=1,
        // Coin=5, face buttons). Never treat those as text input or IME will
        // swallow / briefly pulse them so the emulator never samples a hold.
        if (isMameGameplayKey(event.getKeyCode())) {
            return false;
        }
    ...
    /** Keys that MAME uses as default Start / Coin / SNES face / D-pad bindings. */
    public static boolean isMameGameplayKey(int keyCode) {
        switch (keyCode) {
...
class SDLMain implements Runnable {
    ...
    public void run() {
        ...
        try {
            // Prefer default over DISPLAY priority so SystemUI is less likely to
            // ANR on constrained hosts (Android emulator + heavy netplay).

...
std::error_condition do_open_disk(const emu_options &options, std::initializer_list<std::reference_wrapper<const std::vector<std::string> > > searchpath, const rom_entry *romp, chd_file &chd, std::function<const rom_entry * ()> next_parent)
{
	...
	//JJG: TODO: Support loading CHD with candy mode
...
std::unique_ptr<emu_file> rom_load_manager::open_rom_file(const std::vector<std::string> &paths, std::vector<std::string> &tried, bool has_crc, u32 crc, std::string_view name, std::error_condition &filerr, bool candy)
{
	...
	std::cout << "Image file result: " << filerr.message() << std::endl;

	if (filerr && candy && machine().options().candy()) {

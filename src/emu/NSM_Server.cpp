...
vector<std::shared_ptr<MemoryBlock> > Server::createMemoryBlock(const std::string& name, unsigned char *ptr,int size)
{
  if(blocks.size()==39)
  {
    //throw ("OOPS");
...
void Server::initialSync(const RakNet::RakNetGUID &guid,running_machine *machine)
{
    ...
    )
  {
    ...
    for (const auto& it2 : it->second.oldInputs)
    {
...
void Server::sync(running_machine *machine)
{
  ...
  if(syncCount>0) // The first sync is not sent to clients
  {
    ...
    syncThread = std::shared_ptr<std::thread>(new std::thread(syncProcessor));

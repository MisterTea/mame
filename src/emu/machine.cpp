...
int nvram_size(running_machine &machine) {
	...
	for (device_nvram_interface &nvram : nvram_interface_enumerator(machine.root_device()))
      ...
      if (file.open(machine.nvram_filename(nvram.device()).c_str()))
			{
				retval += file.size();
			}
		}
...
void running_machine::nvram_load()
{
	for (device_nvram_interface &nvram : nvram_interface_enumerator(root_device()))
	{
		...
		if (!netCommon && nvram.nvram_backup_enabled() && !file.open(nvram_filename(nvram.device()))) // JJG: Disable nvram on mamehub for now
		{

...
vector<std::shared_ptr<MemoryBlock> > Client::createMemoryBlock(const std::string& name, unsigned char *ptr,int size)
{
  if(initComplete)
  {
  ...
  do
  {
    ...
    case ID_ADVERTISE_SYSTEM:
    {
      ...
      if(sa2 != RakNet::UNASSIGNED_SYSTEM_ADDRESS)
      {
        ...
        attoseconds_t attosecs = 0;
        memcpy(tmpbuf,&attosecs,sizeof(attosecs));
        tmpbuf += sizeof(attosecs);

        startTime = attotime(secs, attosecs);
...
void Client::revert(running_machine *machine)
{
  ...
  machine->set_machine_time(attotime(syncSeconds, syncAttoseconds));

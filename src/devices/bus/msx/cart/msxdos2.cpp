...
protected:
	...
	// Avoid Android NDK PAGE_SIZE macro (bits/page_size.h).
	static inline constexpr unsigned MSX_PAGE_SIZE = 0x4000;
...
std::error_condition msx_cart_msxdos2_base_device::initialize_cartridge(std::string &message)
{
	...
	m_rombank->configure_entries(0, 4, cart_rom_region()->base(), MSX_PAGE_SIZE);
...
std::error_condition msx_cart_msxdos2j_device::initialize_cartridge(std::string &message)
{
	...
	if (ram_size > 256 * MSX_PAGE_SIZE || (ram_size & (MSX_PAGE_SIZE - 1)) != 0)
	{
	...
	if (ram_size)
	{
		m_bank_mask = device_generic_cart_interface::map_non_power_of_two(
			unsigned(ram_size / MSX_PAGE_SIZE),
			[this] (unsigned entry, unsigned page)
			{
				for (int i = 0; i < 4; i++)
					m_rambank[i]->configure_entry(entry, cart_ram_region()->base() + MSX_PAGE_SIZE * page);
			}
		);
